...
public class GoogleController {
	...
	public RedirectView googleCallback(
		...
		try {
			// 1. 구글 액세스 토큰 요청
			System.out.println("\n📝 [Step 1] 구글 액세스 토큰 요청 중...");
			String googleAccessToken = googleAuthService.getAccessToken(code, state);
			System.out.println("✅ [Step 1] 구글 액세스 토큰 획득 성공");

			// 2. 구글 사용자 정보 조회
			System.out.println("\n📝 [Step 2] 구글 사용자 정보 조회 중...");
			GoogleUserInfo googleUserInfo = googleAuthService.getUserInfo(googleAccessToken);
			...
			// 에러 시 프론트엔드 메인 페이지로 리디렉션
			String errorMessage = URLEncoder.encode("구글 로그인 처리 중 오류가 발생했습니다: " + e.getMessage(), StandardCharsets.UTF_8);
